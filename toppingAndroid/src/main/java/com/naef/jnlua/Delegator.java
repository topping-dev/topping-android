    ...
    protected class DelegateProxy  implements IDelegate,InvocationHandler
    {
        ...
        */
        ...
        public Object invoke(Object[] args) /*   throws IllegalArgumentException,DelegateInvokeException*/
        {
            // validateArgs(args);
            try {
            	//Log.d("Delegator.java", "Invoking function " + getMethod().toString());
                Object ret = getMethod().invoke(getTarget(),args);
                return(ret);
            }
            catch(LuaException lex)
            {
                Log.e("Delegator.java", getMethod().toString());
                Log.e("Delegator.java", lex.toString());
                lex.printStackTrace();
                //Tools.LogException(lex.L, "LuaException", lex);
                return 0;
            }
            catch(Exception ex)
            {
                Log.e("Delegator.java", getMethod().toString());
                Log.e("Delegator.java", ex.toString());
                ex.printStackTrace();
                return 0;
            }
        }
