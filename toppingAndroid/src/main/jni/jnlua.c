...
static void pushlightjavaobject(lua_State *L, jobject object);
static void newjavaobject(lua_State *L, jobject object);
static jobject tojavaobject(lua_State *L, int index, jclass class);
static jobject tolightjavaobject(lua_State *L, int index, jclass class);
...
static int newstate_protected (lua_State *L) {
	...
	lua_pushboolean(L, 0); /* @max: non weak global reference */
	lua_pushcclosure(L, gcjavaobject, 1);
	lua_setfield(L, -2, "__gc");
	// @max: used to be weak reference, but since android less than ics the weak references are almost instantly GC'd we'll get a segfault
...
JNIEXPORT void JNICALL Java_com_naef_jnlua_LuaState_lua_1pushjavaclosure (JNIEnv *env, jobject obj, jobject f, jint i) {
    lua_State *L;
    ...
    if (checkstack(L, JNLUA_MINSTACK)
            && checknotnull(f)) {
        pushjavafunction_f = f;
        lua_pushcclosure(L, pushjavafunction_protected, i);
        JNLUA_PCALL(L, 0, 1);
    }
}
...
JNLUA_THREADLOCAL jobject pushlightjavaobject_object;
static int pushlightjavaobject_protected (lua_State *L) {
	pushlightjavaobject(L, pushlightjavaobject_object);
	return 1;
}
JNIEXPORT void JNICALL Java_com_naef_jnlua_LuaState_lua_1pushlightjavaobject (JNIEnv *env, jobject obj, jobject object) {
	lua_State *L;
	...
	if (checkstack(L, JNLUA_MINSTACK)
			&& checknotnull(object)) {
		pushlightjavaobject_object = object;
		lua_pushcfunction(L, pushlightjavaobject_protected);
		JNLUA_PCALL(L, 0, 1);
	}
}

JNIEXPORT void JNICALL Java_com_naef_jnlua_LuaState_lua_1pushlightuserdata (JNIEnv *env, jobject obj, jobject object) {
	lua_State *L;
	...
	if (checkstack(L, JNLUA_MINSTACK)
			&& checknotnull(object)) {
		lua_pushlightuserdata(L, (*thread_env)->NewGlobalRef(thread_env, object));
	}
}

JNIEXPORT void JNICALL Java_com_naef_jnlua_LuaState_lua_1newjavaobject (JNIEnv *env, jobject obj, jobject object) {
	lua_State *L;
...
JNLUA_THREADLOCAL jobject tolightjavaobject_result;
static int tolightjavaobject_protected (lua_State *L) {
	tolightjavaobject_result = tolightjavaobject(L, 1, NULL);
	return 0;
}
JNIEXPORT jobject JNICALL Java_com_naef_jnlua_LuaState_lua_1tolightjavaobject (JNIEnv *env, jobject obj, jint index) {
	lua_State *L;
	...
	if (checkstack(L, JNLUA_MINSTACK)
			&& checkindex(L, index)) {
		index = lua_absindex(L, index);
		lua_pushcfunction(L, tolightjavaobject_protected);
		lua_pushvalue(L, index);
		JNLUA_PCALL(L, 1, 0);
	}
	return tolightjavaobject_result;
}
...
//DK
JNLUA_THREADLOCAL const char *luaLerror_val;
static int luaLerror_protected (lua_State *L) {
	luaL_error(L, luaLerror_val);
	return 1;
}
JNIEXPORT void JNICALL Java_com_naef_jnlua_LuaState_luaL_1error (JNIEnv *env, jobject obj, jstring err) {
	lua_State *L;
	...
	L = getluathread(obj);
	if (checkstack(L, JNLUA_MINSTACK)
			&& (luaLerror_val = getstringchars(err))) {
		lua_pushcfunction(L, luaLerror_protected);
		JNLUA_PCALL(L, 0, 1);
	}
	if (getglobal_name) {
		releasestringchars(err, luaLerror_val);
	}
}

/* setasset() */
JNIEXPORT void JNICALL Java_com_naef_jnlua_LuaState_setasset (JNIEnv *env, jobject obj, jobject assetMgr) {
...
static void pushlightjavaobject (lua_State *L, jobject object) {
	...
	user_data = (jobject *) lua_newuserdata(L, sizeof(jobject));
	luaL_getmetatable(L, JNLUA_OBJECT);
	jobject user_data = (*thread_env)->NewGlobalRef(thread_env, object);
	if (!*user_data) {
		lua_pushliteral(L, "JNI error: NewGlobalRef() failed pushing Java object");
		lua_error(L);
	}
...
static void newjavaobject (lua_State *L, jobject object) {
    ...
    void **ptr_hold = (void **) lua_newuserdata(L, sizeof(jobject));
	*ptr_hold = (*thread_env)->NewGlobalRef(thread_env, object);
}
...
JNIEXPORT jlong JNICALL Java_com_naef_jnlua_LuaState_hvaluew (JNIEnv *env, jobject obj, jlong val) {
	lua_State* L;

	JNLUA_ENV(env);
	L = getluathread(obj);
	if (checkstack(L, JNLUA_MINSTACK)) {
		TValue* arg = (TValue*)val;
		return (jlong)hvalue(arg);
	}

	return 0;
}

JNIEXPORT jint JNICALL Java_com_naef_jnlua_LuaState_sizenodew (JNIEnv *env, jobject obj, jlong val) {
	lua_State* L;
	...
	if (checkstack(L, JNLUA_MINSTACK)) {
		Table* arg = (Table*)val;
		return (jint)sizenode(arg);
	}

	return 0;
}

JNIEXPORT jlong JNICALL Java_com_naef_jnlua_LuaState_gnodew (JNIEnv *env, jobject obj, jlong val, jint index) {
	lua_State* L;
	...
	if (checkstack(L, JNLUA_MINSTACK)) {
		Table* arg = (Table*)val;
		return (jlong)gnode(arg, index);
	}

	return 0;
}

JNIEXPORT jlong JNICALL Java_com_naef_jnlua_LuaState_key2tvalw (JNIEnv *env, jobject obj, jlong val) {
	lua_State* L;
	...
	if (checkstack(L, JNLUA_MINSTACK)) {
		Node* arg = (Node*)val;
		return (jlong)key2tval(arg);
	}

	return 0;
}

JNIEXPORT jint JNICALL Java_com_naef_jnlua_LuaState_ttypew (JNIEnv *env, jobject obj, jlong val) {
	lua_State* L;
	...
	if (checkstack(L, JNLUA_MINSTACK)) {
		TValue* arg = (TValue*)val;
		return (jint)ttype(arg);
	}

	return 0;
}

JNIEXPORT jstring JNICALL Java_com_naef_jnlua_LuaState_svaluew (JNIEnv *env, jobject obj, jlong val) {
	lua_State* L;
	...
	if (checkstack(L, JNLUA_MINSTACK)) {
		TValue* arg = (TValue*)val;
		jstring js = (*env)->NewStringUTF(env, svalue(arg));
		return js;
	}
...
JNIEXPORT jdouble JNICALL Java_com_naef_jnlua_LuaState_nvaluew (JNIEnv *env, jobject obj, jlong val) {
	lua_State* L;
	...
	if (checkstack(L, JNLUA_MINSTACK)) {
		TValue* arg = (TValue*)val;
		return (jdouble)nvalue(arg);
	}

	return 0;
}

JNIEXPORT jlong JNICALL Java_com_naef_jnlua_LuaState_pvaluew (JNIEnv *env, jobject obj, jlong val) {
	lua_State* L;
	...
	if (checkstack(L, JNLUA_MINSTACK)) {
		TValue* arg = (TValue*)val;
		return (jlong)pvalue(arg);
	}

	return 0;
}

JNIEXPORT jlong JNICALL Java_com_naef_jnlua_LuaState_luaH_1getw (JNIEnv *env, jobject obj, jlong table, jlong val) {
	lua_State* L;

	JNLUA_ENV(env);
	L = getluathread(obj);
	if (checkstack(L, JNLUA_MINSTACK)) {
		Table* argTable = (Table*)table;
		TValue* arg = (TValue*)val;
		return (jlong)luaH_get(argTable, arg);
	}
